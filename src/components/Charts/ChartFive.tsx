...
const options: ApexOptions = {
  ...
  yaxis: {
    title: {
      text: "Emission", // Set the Y-axis label
      style: {
...
const ChartFive: React.FC<ChartFiveProps> = ({ countryName,series }) => {
  return (
      ...
      <div className="mb-4 justify-between gap-4 sm:flex">
        <div>
          <h4 className="text-xl font-semibold text-black dark:text-white">
           {countryName} Total Emission Overview
          </h4>
        </div>
      </div>
      <ReactApexChart options={options} series={series} type="bar" height={335} />
      <h2 className="flex justify-center font-satoshi text-[14px] font-semibold">Category</h2>
    </div>
  );
};
